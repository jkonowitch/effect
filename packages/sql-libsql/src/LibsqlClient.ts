...
export const make = (
  ...
  Effect.gen(function*() {
    ...
    const makeConnection = Effect.gen(function*() {
      let db: Libsql.Client

      if (options._tag === "Config") {
        db = Libsql.createClient(options as Libsql.Config)
        yield* Effect.addFinalizer(() => Effect.sync(() => db.close()))
      } else {
        db = options.liveClient
      }
      ...
      const run = (
        ...
        Effect.tryPromise({
          try: () => db.execute({ sql, args: params as Array<Libsql.InValue> }).then((results) => results.rows),
      ...
      const runRaw = (
        ...
        Effect.tryPromise({
          try: () => db.execute({ sql, args: params as Array<Libsql.InValue> }),
          catch: (cause) => new SqlError({ cause, message: "Failed to execute statement" })
        })
